...
public partial class ChartPlayer : Control
{
    ...
    public override void _Ready()
    {
        ...
        noteWidthScale = Size.X * 1.3f / tapTexture.GetWidth();
        GD.Print($"Size.X:{Size.X}, tapTexture.GetSize().X:{tapTexture.GetSize().X}, noteWidthScale:{noteWidthScale}");
...
public partial class JudgeLineNode : Node2D
{
    ...
    public void UpdateLine(double gameTime)
    {
        ...
        // 我们需要综合所有事件层，第0层为主层
        {
            ...
            _currentAlpha = ChartDataHelper.InterpolateEvent(layer.AlphaEvents, gameTime, 0);
            _currentSpeed = ChartDataHelper.InterpolateEvent(layer.SpeedEvents, gameTime, 0);
        }
        ...
        for(int i = 1; i <= 3; i++)
        {
            if(i > _data.EventLayers.Length - 1) break;
            EventLayer layer = _data.EventLayers[i];
        ...
        //调整颜色和透明度
        _currentAlpha = Math.Clamp(_currentAlpha, 0f, 255f);
